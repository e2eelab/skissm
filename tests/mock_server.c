...
static group_data group_data_set[group_data_max] = {
    {NULL, NULL, 0, NULL}, {NULL, NULL, 0, NULL}, {NULL, NULL, 0, NULL},
    {NULL, NULL, 0, NULL}, {NULL, NULL, 0, NULL}, {NULL, NULL, 0, NULL},
...
/* NOTE:
1. count the number of other involving members
2. copy each involving member's address into "user_addresses"
*/

/*---TODO: get number without copying addresses-------------------------------*/

static size_t find_num_user_addresses(const char *user_id) {
...
/*------------------------------------*/
static size_t find_user_addresses(const char *user_id,
    ...
    *user_addresses = (Skissm__E2eeAddress **)malloc(
        sizeof(Skissm__E2eeAddress *) * user_addresses_num);
    uint8_t j = 0;
    for (i = 0; i < user_data_max; i++) {
        if (user_data_set[i].address != NULL) {
            if (safe_strcmp(user_data_set[i].address->user->user_id, user_id))
                copy_address_from_address(&((*user_addresses)[j++]),
                                          user_data_set[i].address);
        }
    }
    return user_addresses_num;
}

static size_t find_friend_addresses(uint8_t user_index,
                                    Skissm__E2eeAddress ***friend_addresses) {
    size_t friends_num = 0;
    uint8_t i;
    for (i = 0; i < user_data_max; i++) {
        if (session_record[user_index][i] == true) {
            if (!safe_strcmp(user_data_set[user_index].address->user->user_id,
                             user_data_set[i].address->user->user_id))
                friends_num++;
        }
    }
    *friend_addresses = (Skissm__E2eeAddress **)malloc(
        sizeof(Skissm__E2eeAddress *) * friends_num);
    uint8_t j = 0;
    for (i = 0; i < user_data_max; i++) {
        if (session_record[user_index][i] == true) {
            if (!safe_strcmp(user_data_set[user_index].address->user->user_id,
                             user_data_set[i].address->user->user_id))
                copy_address_from_address(&((*friend_addresses)[j++]),
                                          user_data_set[i].address);
        }
    }
    return friends_num;
...
static size_t find_group_data(uint8_t user_index,
    ...
    *group_info_list =
        (Skissm__GroupInfo **)malloc(sizeof(Skissm__GroupInfo *) * group_num);
    uint8_t j = 0;
    for (i = 0; i < group_data_max; i++) {
        if (group_record[user_index][i] == true) {
            (*group_info_list)[j] =
            ...
            (*group_info_list)[j]->group_name =
                strdup(cur_group_data->group_name);
            copy_address_from_address(&((*group_info_list)[j]->group_address),
                                      cur_group_data->group_address);
            (*group_info_list)[j]->n_group_members =
                cur_group_data->group_members_num;
            copy_group_members(&((*group_info_list)[j]->group_members),
                               cur_group_data->group_members,
                               cur_group_data->group_members_num);
            // done
            j++;
        }
...
void mock_server_end() {
    ...
    for (i = 0; i < user_data_set_insert_pos; i++) {
        ...
        skissm__identity_key_public__free_unpacked(
            user_data_set[i].identity_key_public, NULL);
        skissm__signed_pre_key_public__free_unpacked(
            user_data_set[i].signed_pre_key_public, NULL);
        for (j = 0; j < user_data_set[i].n_one_time_pre_keys; j++) {
            Skissm__OneTimePreKeyPublic *cur_opk =
                user_data_set[i].one_time_pre_keys[j];
            if (cur_opk != NULL) {
                skissm__one_time_pre_key_public__free_unpacked(cur_opk, NULL);
                cur_opk = NULL;
            }
        }
        free_mem((void **)&(user_data_set[i].one_time_pre_keys),
                 sizeof(Skissm__OneTimePreKeyPublic *) *
                     user_data_set[i].n_one_time_pre_keys);
    ...
    for (i = 0; i < group_data_set_insert_pos; i++) {
        skissm__e2ee_address__free_unpacked(group_data_set[i].group_address,
                                            NULL);
        ...
        for (j = 0; j < group_data_set[i].group_members_num; j++) {
            Skissm__GroupMember *cur_group_member =
                group_data_set[i].group_members[j];
            if (cur_group_member != NULL) {
                skissm__group_member__free_unpacked(cur_group_member, NULL);
                cur_group_member = NULL;
            }
        }
        free_mem((void **)&(group_data_set[i].group_members),
                 sizeof(Skissm__GroupMember *) *
                     group_data_set[i].group_members_num);
...
Skissm__RegisterUserResponse *mock_register_user(
    ...
    Skissm__E2eeAddress **other_device_addresses, **friend_addresses,
        **receiver_addresses;
    Skissm__GroupInfo **group_info_list;
    size_t other_device_num = 0, friends_num = 0, receiver_num = 0,
           group_num = 0;
    if (client_data != NULL) {
    ...
    copy_ik_public_from_ik_public(&(cur_data->identity_key_public),
                                  request->identity_key_public);
    copy_spk_public_from_spk_public(&(cur_data->signed_pre_key_public),
                                    request->signed_pre_key_public);
    cur_data->n_one_time_pre_keys = request->n_one_time_pre_keys;
    cur_data->one_time_pre_keys = (Skissm__OneTimePreKeyPublic **)malloc(
        sizeof(Skissm__OneTimePreKeyPublic *) * cur_data->n_one_time_pre_keys);
    size_t i, j;
    for (i = 0; i < cur_data->n_one_time_pre_keys; i++) {
        copy_opk_public_from_opk_public(&(cur_data->one_time_pre_keys[i]),
                                        request->one_time_pre_keys[i]);
    }
    ...
    if (client_data != NULL) {
        ...
        cur_data->address->user->user_id =
            strdup(client_data->address->user->user_id);
    }

    user_data_set_insert_pos++;

    Skissm__RegisterUserResponse *response =
        (Skissm__RegisterUserResponse *)malloc(
    ...
    if (client_data != NULL) {
        receiver_addresses = (Skissm__E2eeAddress **)malloc(
            sizeof(Skissm__E2eeAddress *) * receiver_num);
        // other devices
        response->n_other_device_addresses = other_device_num;
        response->other_device_addresses = (Skissm__E2eeAddress **)malloc(
            sizeof(Skissm__E2eeAddress *) * other_device_num);
        for (i = 0; i < other_device_num; i++) {
            copy_address_from_address(&((response->other_device_addresses)[i]),
                                      other_device_addresses[i]);
            copy_address_from_address(&(receiver_addresses[i]),
                                      other_device_addresses[i]);
        }
        // other users
        response->n_other_user_addresses = friends_num;
        response->other_user_addresses = (Skissm__E2eeAddress **)malloc(
            sizeof(Skissm__E2eeAddress *) * friends_num);
        for (i = 0; i < friends_num; i++) {
            copy_address_from_address(&((response->other_user_addresses)[i]),
                                      friend_addresses[i]);
            copy_address_from_address(
                &(receiver_addresses[other_device_num + i]),
                friend_addresses[i]);
        }
        // group
        response->n_group_info_list = group_num;
        response->group_info_list = (Skissm__GroupInfo **)malloc(
            sizeof(Skissm__GroupInfo *) * group_num);
        for (i = 0; i < group_num; i++) {
            (response->group_info_list)[i] =
                (Skissm__GroupInfo *)malloc(sizeof(Skissm__GroupInfo));
            ...
            copy_address_from_address(&(cur_group->group_address),
                                      group_info_list[i]->group_address);
            cur_group->n_group_members = group_info_list[i]->n_group_members;
            copy_group_members(&(cur_group->group_members),
                               group_info_list[i]->group_members,
                               group_info_list[i]->n_group_members);
        }
    }

    // send the new device message to other devices and users
    for (j = 0; j < receiver_num; j++) {
        Skissm__ProtoMsg *proto_msg =
            (Skissm__ProtoMsg *)malloc(sizeof(Skissm__ProtoMsg));
        ...
        proto_msg->payload_case =
            SKISSM__PROTO_MSG__PAYLOAD_NEW_USER_DEVICE_MSG;
        proto_msg->new_user_device_msg = (Skissm__NewUserDeviceMsg *)malloc(
            sizeof(Skissm__NewUserDeviceMsg));
        skissm__new_user_device_msg__init(proto_msg->new_user_device_msg);
        copy_address_from_address(
            &(proto_msg->new_user_device_msg->user_address), cur_data->address);
        proto_msg->new_user_device_msg->n_group_info_list = group_num;
        proto_msg->new_user_device_msg->group_info_list =
            (Skissm__GroupInfo **)malloc(sizeof(Skissm__GroupInfo *) *
                                         group_num);
        for (i = 0; i < group_num; i++) {
            (proto_msg->new_user_device_msg->group_info_list)[i] =
                (Skissm__GroupInfo *)malloc(sizeof(Skissm__GroupInfo));
            Skissm__GroupInfo *cur_group =
                (proto_msg->new_user_device_msg->group_info_list)[i];
            skissm__group_info__init(cur_group);
            cur_group->group_name = strdup(group_info_list[i]->group_name);
            copy_address_from_address(&(cur_group->group_address),
    ...
    if (client_data != NULL) {
        for (i = 0; i < other_device_num; i++) {
            skissm__e2ee_address__free_unpacked(other_device_addresses[i],
                                                NULL);
        }
        free_mem((void **)&other_device_addresses,
                 sizeof(Skissm__E2eeAddress *) * other_device_num);
    }
...
Skissm__GetPreKeyBundleResponse *mock_get_pre_key_bundle(
    ...
    if ((request->device_id)[0] == '\0') {
        for (i = 0; i < user_data_set_insert_pos; i++) {
            ...
            if ((cur_address != NULL) &&
                compare_user_id(cur_address, request->user_id,
                                request->domain)) {
    ...
    response = (Skissm__GetPreKeyBundleResponse *)malloc(
    ...
    response->pre_key_bundles = (Skissm__PreKeyBundle **)malloc(
        sizeof(Skissm__PreKeyBundle *) * user_device_num);
    ...
    for (j = 0; j < user_device_num; j++) {
        ...
        response->pre_key_bundles[j] =
            (Skissm__PreKeyBundle *)malloc(sizeof(Skissm__PreKeyBundle));
        skissm__pre_key_bundle__init(response->pre_key_bundles[j]);

        response->pre_key_bundles[j]->e2ee_pack_id =
            strdup(cur_data->e2ee_pack_id);
        copy_address_from_address(&(response->pre_key_bundles[j]->user_address),
                                  cur_data->address);
        copy_ik_public_from_ik_public(
            &(response->pre_key_bundles[j]->identity_key_public),
            cur_data->identity_key_public);
        copy_spk_public_from_spk_public(
            &(response->pre_key_bundles[j]->signed_pre_key_public),
            cur_data->signed_pre_key_public);

        size_t k;
        for (k = 0; k < cur_data->n_one_time_pre_keys; k++) {
            if (cur_data->one_time_pre_keys[k]) {
                copy_opk_public_from_opk_public(
                    &(response->pre_key_bundles[j]->one_time_pre_key_public),
                    cur_data->one_time_pre_keys[k]);
                break;
            }
        }
        // release the one-time pre-key
        skissm__one_time_pre_key_public__free_unpacked(
...
Skissm__InviteResponse *mock_invite(Skissm__E2eeAddress *from, const char *auth,
                                    Skissm__InviteRequest *request) {
    Skissm__InviteMsg *invite_msg = request->msg;
    size_t invite_msg_data_len =
        skissm__invite_msg__get_packed_size(invite_msg);
    ...
    proto_msg->invite_msg =
        skissm__invite_msg__unpack(NULL, invite_msg_data_len, invite_msg_data);

    send_proto_msg(proto_msg);

    // prepare response
    Skissm__InviteResponse *response =
        (Skissm__InviteResponse *)malloc(sizeof(Skissm__InviteResponse));
...
Skissm__AcceptResponse *mock_accept(Skissm__E2eeAddress *from, const char *auth,
                                    Skissm__AcceptRequest *request) {
    Skissm__AcceptMsg *accept_msg = request->msg;
    size_t accept_msg_data_len =
        skissm__accept_msg__get_packed_size(accept_msg);
    ...
    proto_msg->accept_msg =
        skissm__accept_msg__unpack(NULL, accept_msg_data_len, accept_msg_data);
    ...
    // prepare response
    Skissm__AcceptResponse *response =
        (Skissm__AcceptResponse *)malloc(sizeof(Skissm__AcceptResponse));
...
Skissm__F2fInviteResponse *mock_f2f_invite(Skissm__E2eeAddress *from,
                                           const char *auth,
    ...
    while (user_data_find < user_data_set_insert_pos) {
        if ((user_data_set[user_data_find].address) && (f2f_invite_msg->to) &&
            compare_address(user_data_set[user_data_find].address,
                            f2f_invite_msg->to)) {
            break;
        }
        user_data_find++;
    ...
    if (user_data_find == user_data_set_insert_pos) {
        Skissm__F2fInviteResponse *response =
            (Skissm__F2fInviteResponse *)malloc(
                sizeof(Skissm__F2fInviteResponse));
        skissm__f2f_invite_response__init(response);
        response->code =
            SKISSM__RESPONSE_CODE__RESPONSE_CODE_INTERNAL_SERVER_ERROR;
        return response;
    }
    ...
    // pack
    size_t f2f_invite_msg_data_len =
        skissm__f2f_invite_msg__get_packed_size(f2f_invite_msg);
    ...
    proto_msg->f2f_invite_msg = skissm__f2f_invite_msg__unpack(
        NULL, f2f_invite_msg_data_len, f2f_invite_msg_data);

    send_proto_msg(proto_msg);

    // prepare response
    Skissm__F2fInviteResponse *response =
        (Skissm__F2fInviteResponse *)malloc(sizeof(Skissm__F2fInviteResponse));
    skissm__f2f_invite_response__init(response);
    copy_spk_public_from_spk_public(&(response->spk_public),
                                    cur_data->signed_pre_key_public);
    response->code = SKISSM__RESPONSE_CODE__RESPONSE_CODE_OK;
...
Skissm__F2fAcceptResponse *mock_f2f_accept(Skissm__E2eeAddress *from,
                                           const char *auth,
                                           Skissm__F2fAcceptRequest *request) {
    Skissm__F2fAcceptMsg *f2f_accept_msg = request->msg;
    size_t f2f_accept_msg_data_len =
        skissm__f2f_accept_msg__get_packed_size(f2f_accept_msg);
    ...
    proto_msg->f2f_accept_msg = skissm__f2f_accept_msg__unpack(
        NULL, f2f_accept_msg_data_len, f2f_accept_msg_data);

    send_proto_msg(proto_msg);

    // prepare response
    Skissm__F2fAcceptResponse *response =
        (Skissm__F2fAcceptResponse *)malloc(sizeof(Skissm__F2fAcceptResponse));
...
Skissm__PublishSpkResponse *mock_publish_spk(
    ...
    if (user_data_find == user_data_set_insert_pos) {
        Skissm__PublishSpkResponse *response =
            (Skissm__PublishSpkResponse *)malloc(
                sizeof(Skissm__PublishSpkResponse));
    ...
    // release old memory
    skissm__signed_pre_key_public__free_unpacked(
        cur_data->signed_pre_key_public, NULL);
    // copy new data
    copy_spk_public_from_spk_public(&(cur_data->signed_pre_key_public),
                                    request->signed_pre_key_public);

    Skissm__PublishSpkResponse *response = (Skissm__PublishSpkResponse *)malloc(
        sizeof(Skissm__PublishSpkResponse));
    skissm__publish_spk_response__init(response);
    response->code = SKISSM__RESPONSE_CODE__RESPONSE_CODE_OK;
...
Skissm__SupplyOpksResponse *mock_supply_opks(
    ...
    temp = (Skissm__OneTimePreKeyPublic **)malloc(
        sizeof(Skissm__OneTimePreKeyPublic *) * cur_data->n_one_time_pre_keys);
    size_t i;
    for (i = 0; i < old_num; i++) {
        copy_opk_public_from_opk_public(&(temp[i]),
                                        cur_data->one_time_pre_keys[i]);
        skissm__one_time_pre_key_public__free_unpacked(
    ...
    for (i = old_num; i < cur_data->n_one_time_pre_keys; i++) {
        copy_opk_public_from_opk_public(
            &(cur_data->one_time_pre_keys[i]),
            request->one_time_pre_key_public[i - old_num]);
    }

    // prepare response
    Skissm__SupplyOpksResponse *response = (Skissm__SupplyOpksResponse *)malloc(
        sizeof(Skissm__SupplyOpksResponse));
...
Skissm__SendOne2oneMsgResponse *mock_send_one2one_msg(
    ...
    if ((e2ee_msg->to->user->device_id)[0] != '\0') {
        ...
        proto_msg->e2ee_msg =
            skissm__e2ee_msg__unpack(NULL, e2ee_msg_data_len, e2ee_msg_data);
        ...
        size_t to_address_num =
            find_user_addresses(e2ee_msg->to->user->user_id, &to_addresses);
        size_t i;
        for (i = 0; i < to_address_num; i++) {
            ...
            proto_msg->payload_case = SKISSM__PROTO_MSG__PAYLOAD_E2EE_MSG;
            proto_msg->e2ee_msg = skissm__e2ee_msg__unpack(
                NULL, e2ee_msg_data_len, e2ee_msg_data);
    ...
    Skissm__SendOne2oneMsgResponse *response =
        (Skissm__SendOne2oneMsgResponse *)malloc(
            sizeof(Skissm__SendOne2oneMsgResponse));
...
Skissm__CreateGroupResponse *mock_create_group(
    ...
    copy_address_from_address(&(create_group_msg->group_info->group_address),
                              cur_group_data->group_address);

    // init response to take the copy of member_id
    Skissm__CreateGroupResponse *response =
        (Skissm__CreateGroupResponse *)malloc(
    ...
    copy_address_from_address(&(sender_address),
                              create_group_msg->sender_address);
    ...
    // TODO: should be free when member is removed? and send new msg?
    // 2d array
    Skissm__GroupMemberID **common_member_ids =
        (Skissm__GroupMemberID **)malloc(to_member_addresses_total_num *
                                         sizeof(Skissm__GroupMemberID *));
    ...
    for (i = 0; i < group_data_set[group_data_set_insert_pos].group_members_num;
        ...
        Skissm__E2eeAddress **to_member_addresses = NULL;
        to_member_addresses_num =
            find_user_addresses(member_user_id, &to_member_addresses);

        for (j = 0; j < to_member_addresses_num; j++) {
            Skissm__E2eeAddress *to_member_address = to_member_addresses[j];

            // member_pos finds the index of specific member in user_data_set
            uint8_t member_pos = find_address(to_member_address);
            // TODO: 因為下面在SEND MSG到每個團體成員時，又需要call
            // "find_address" O(n) ,需要開空間儲存嗎? 但如果單純用 array,
            // 如果成員太多可能會出現sparse 最好是用 map, 但有必要嗎? 必須手刻

            copy_address_from_address(
                &(common_member_ids[j]->group_member_address),
                to_member_address);
            copy_protobuf_from_protobuf(
                &(common_member_ids[j]->public_key),
                &(user_data_set[member_pos]
                      .identity_key_public->sign_public_key));
        }
    }

    copy_members_id(&(create_group_msg->member_ids), common_member_ids,
                    to_member_addresses_total_num);

    copy_members_id(&(response->member_ids), common_member_ids,
                    to_member_addresses_total_num);

    /*--------------------------------------*/

    size_t create_group_msg_data_len =
        skissm__create_group_msg__get_packed_size(create_group_msg);
    ...
    copy_group_members(&(cur_group_data->group_members),
                       group_info->group_members, group_info->n_group_members);
    ...
    for (i = 0; i < group_data_set[group_data_set_insert_pos].group_members_num;
        ...
        for (j = 0; j < to_member_addresses_num; j++) {
            ...
            proto_msg->payload_case =
                SKISSM__PROTO_MSG__PAYLOAD_CREATE_GROUP_MSG;
            proto_msg->create_group_msg = skissm__create_group_msg__unpack(
                NULL, create_group_msg_data_len, create_group_msg_data);
    ...
    // prepare response
    copy_address_from_address(&(response->group_address),
                              cur_group_data->group_address);
...
Skissm__AddGroupMembersResponse *mock_add_group_members(
    ...
    size_t add_group_members_msg_data_len =
        skissm__add_group_members_msg__get_packed_size(add_group_members_msg);
    uint8_t add_group_members_msg_data[add_group_members_msg_data_len];
    skissm__add_group_members_msg__pack(add_group_members_msg,
                                        add_group_members_msg_data);
    ...
    while (group_data_find < group_data_set_insert_pos) {
        if ((group_data_set[group_data_find].group_address) &&
            ...
            compare_address(group_data_set[group_data_find].group_address,
                            add_group_members_msg->group_info->group_address)) {
            break;
        }
        group_data_find++;
    ...
    if (group_data_find == group_data_set_insert_pos) {
        Skissm__AddGroupMembersResponse *response =
            (Skissm__AddGroupMembersResponse *)malloc(
    ...
    for (i = 0; i < group_data_set[group_data_find].group_members_num; i++) {
        ...
        for (j = 0; j < to_member_addresses_num; j++) {
            ...
            copy_address_from_address(&(proto_msg->to), to_member_address);
            proto_msg->payload_case =
                SKISSM__PROTO_MSG__PAYLOAD_ADD_GROUP_MEMBERS_MSG;
            proto_msg->add_group_members_msg =
                skissm__add_group_members_msg__unpack(
                    NULL, add_group_members_msg_data_len,
                    add_group_members_msg_data);
    ...
    response->group_members = (Skissm__GroupMember **)malloc(
        sizeof(Skissm__GroupMember *) * new_group_members_num);
    for (i = 0; i < new_group_members_num; i++) {
        (response->group_members)[i] =
            (Skissm__GroupMember *)malloc(sizeof(Skissm__GroupMember));
        skissm__group_member__init((response->group_members)[i]);
        copy_group_member(&((response->group_members)[i]),
                          (cur_group_data->group_members)[i]);
    }
...
Skissm__RemoveGroupMembersResponse *mock_remove_group_members(
    ...
    size_t remove_group_members_msg_data_len =
        skissm__remove_group_members_msg__get_packed_size(
            remove_group_members_msg);
    uint8_t remove_group_members_msg_data[remove_group_members_msg_data_len];
    skissm__remove_group_members_msg__pack(remove_group_members_msg,
                                           remove_group_members_msg_data);
    ...
    if (group_data_find == group_data_set_insert_pos) {
        Skissm__RemoveGroupMembersResponse *response =
            (Skissm__RemoveGroupMembersResponse *)malloc(
    ...
    size_t new_group_members_num =
        original_group_members_num - request->msg->n_removing_members;
    cur_group_data->group_members_num = new_group_members_num;

    Skissm__GroupMember **temp_group_members = (Skissm__GroupMember **)malloc(
        sizeof(Skissm__GroupMember *) * new_group_members_num);
    size_t i = 0, j = 0;
    ...
    while (i < new_group_members_num && j < original_group_members_num) {
        ...
        if (cur_removing_member_num < request->msg->n_removing_members) {
            if (!safe_strcmp(
                    (cur_group_data->group_members)[j]->user_id,
                    request->msg->removing_members[cur_removing_member_num]
                        ->user_id)) {
                copy_group_member(&(temp_group_members[i]),
                                  (cur_group_data->group_members)[j]);
                i++;
                j++;
            } else {
                j++;
    ...
    Skissm__E2eeAddress *sender_address =
        remove_group_members_msg->sender_address;
    const char *sender_user_id = sender_address->user->user_id;
    for (i = 0; i < original_group_members_num; i++) {
        ...
        for (j = 0; j < to_member_addresses_num; j++) {
            ...
            copy_address_from_address(&(proto_msg->to), to_member_address);
            proto_msg->payload_case =
                SKISSM__PROTO_MSG__PAYLOAD_REMOVE_GROUP_MEMBERS_MSG;
            proto_msg->remove_group_members_msg =
                skissm__remove_group_members_msg__unpack(
                    NULL, remove_group_members_msg_data_len,
                    remove_group_members_msg_data);
    ...
    for (i = 0; i < original_group_members_num; i++) {
        skissm__group_member__free_unpacked((cur_group_data->group_members)[i],
                                            NULL);
...
Skissm__SendGroupMsgResponse *mock_send_group_msg(
    ...
    if (group_data_find == group_data_set_insert_pos) {
        Skissm__SendGroupMsgResponse *response =
            (Skissm__SendGroupMsgResponse *)malloc(
                sizeof(Skissm__SendGroupMsgResponse));
...
Skissm__ConsumeProtoMsgResponse *mock_consume_proto_msg(
    ...
    size_t request_data_len =
        skissm__consume_proto_msg_request__get_packed_size(request);
    uint8_t *request_data = (uint8_t *)malloc(request_data_len);
    skissm__consume_proto_msg_request__pack(request, request_data);

    Skissm__ConsumeProtoMsgResponse *response =
        (Skissm__ConsumeProtoMsgResponse *)malloc(
