...
static const char *N_GROUP_SESSION_LOAD_INBOUND =
...
static const char *GROUP_SESSIONS_LOAD_INBOUND =
    "SELECT GROUP_DATA FROM GROUP_SESSION "
    "INNER JOIN ADDRESS as a1 "
    "ON GROUP_SESSION.OWNER = a1.ID "
    "INNER JOIN ADDRESS as a2 "
...
static const char *N_RESENDING_PLAINTEXT_DATA_LOAD = "SELECT COUNT(*) "
                                                     "FROM PENDING_PLAINTEXT_DATA "
                                                     "INNER JOIN ADDRESS AS a1 "
                                                     "ON PENDING_PLAINTEXT_DATA.FROM_ADDRESS = a1.ID "
                                                     "WHERE a1.DOMAIN is (?) AND a1.USER_ID is (?) AND a1.DEVICE_ID is (?) "
...
static const char *RESENDING_PLAINTEXT_DATA_LOAD = "SELECT "
                                                   "TO_ADDRESS.DOMAIN, "
                                                   "TO_ADDRESS.USER_ID, "
...
static void load_identity_key_asym(
    ...
    *asym_key_pair = (Skissm__KeyPair *)malloc(sizeof(Skissm__KeyPair));
    ...
    copy_protobuf_from_array(
        &((*asym_key_pair)->public_key),
        (uint8_t *)sqlite3_column_blob(stmt, 0),
        sqlite3_column_bytes(stmt, 0)
    );
    copy_protobuf_from_array(
        &((*asym_key_pair)->private_key),
...
static void load_identity_key_sign(
    sqlite_int64 identity_key_id,
    ...
    *sign_key_pair = (Skissm__KeyPair *)malloc(sizeof(Skissm__KeyPair));
    ...
    // load
    copy_protobuf_from_array(
        &((*sign_key_pair)->public_key),
        (uint8_t *)sqlite3_column_blob(stmt, 0),
        sqlite3_column_bytes(stmt, 0)
    );
    copy_protobuf_from_array(
        &((*sign_key_pair)->private_key),
        (uint8_t *)sqlite3_column_blob(stmt, 1),
        sqlite3_column_bytes(stmt, 1)
    );
...
void load_signed_pre_key_pair(
    ...
    *signed_pre_key =
    ...
    copy_protobuf_from_array(
        &((*signed_pre_key)->key_pair->private_key),
...
uint32_t load_one_time_pre_keys(
    ...
    for (int i = 0; i < n_one_time_pre_keys; i++) {
        ...
        copy_protobuf_from_array(
            &((*one_time_pre_keys)[i]->key_pair->public_key),
            (uint8_t *)sqlite3_column_blob(stmt, 2),
            sqlite3_column_bytes(stmt, 2)
        );
        copy_protobuf_from_array(
            &((*one_time_pre_keys)[i]->key_pair->private_key),
            (uint8_t *)sqlite3_column_blob(stmt, 3),
            sqlite3_column_bytes(stmt, 3)
        );
    }
...
void insert_account_identity_key_id(
    uint64_t account_id, sqlite_int64 identity_key_id
) {
    // prepare
    sqlite3_stmt *stmt;
...
void insert_account_signed_pre_key_id(
    uint64_t account_id, sqlite_int64 signed_pre_key_id
) {
    // prepare
    sqlite3_stmt *stmt;
...
static void delete_account_signed_pre_key(
    uint64_t account_id, sqlite_int64 signed_pre_key_id
) {
    sqlite3_stmt *stmt;
...
void remove_one_time_pre_key(
    uint64_t account_id, uint32_t one_time_pre_key_id
) {
    // prepare
    sqlite3_stmt *stmt;
...
void load_inbound_group_session(
    ...
    sqlite3_bind_text(stmt, 3, receiver_address->domain, -1, SQLITE_TRANSIENT);
    sqlite3_bind_text(stmt, 4, receiver_address->user->device_id, -1, SQLITE_TRANSIENT);
...
int load_n_inbound_group_sessions(
    ...
    // prepare
    sqlite3_stmt *stmt;
    sqlite_prepare(N_GROUP_SESSION_LOAD_INBOUND, &stmt);
    sqlite3_bind_text(stmt, 1, owner->domain, -1, SQLITE_TRANSIENT);
    sqlite3_bind_text(stmt, 2, owner->user->user_id, -1, SQLITE_TRANSIENT);
    sqlite3_bind_text(stmt, 3, owner->user->device_id, -1, SQLITE_TRANSIENT);
    sqlite3_bind_text(stmt, 4, group_address->group->group_id, -1, SQLITE_TRANSIENT);
    ...
    // load
    int n_inbound_group_sessions = (int)sqlite3_column_int(stmt, 0);
...
size_t load_inbound_group_sessions(
    ...
    (*inbound_group_sessions) = (Skissm__GroupSession **)malloc(
        n_inbound_group_sessions * sizeof(Skissm__GroupSession *));

    // prepare
    sqlite3_stmt *stmt;
    sqlite_prepare(GROUP_SESSIONS_LOAD_INBOUND, &stmt);
    sqlite3_bind_text(stmt, 1, owner->domain, -1, SQLITE_TRANSIENT);
    sqlite3_bind_text(stmt, 2, owner->user->user_id, -1, SQLITE_TRANSIENT);
    sqlite3_bind_text(stmt, 3, owner->user->device_id, -1, SQLITE_TRANSIENT);
    sqlite3_bind_text(stmt, 4, group_address->group->group_id, -1, SQLITE_TRANSIENT);
    ...
    // step
    for (int i = 0; i < n_inbound_group_sessions; i++) {
        sqlite3_step(stmt);
        ...
        // unpack
        (*inbound_group_sessions)[i] = skissm__group_session__unpack(NULL, group_session_data_len, group_session_data);
    }
...
void unload_inbound_group_session(
    ...
    sqlite3_bind_text(stmt, 2, receiver_address->user->device_id, -1, SQLITE_TRANSIENT);
...
int load_n_resending_plaintext(
    Skissm__E2eeAddress *from_address
) {
    // prepare
    sqlite3_stmt *stmt;
    sqlite_prepare(N_RESENDING_PLAINTEXT_DATA_LOAD, &stmt);
    sqlite3_bind_text(stmt, 1, from_address->domain, -1, SQLITE_TRANSIENT);
    sqlite3_bind_text(stmt, 2, from_address->user->user_id, -1, SQLITE_TRANSIENT);
    sqlite3_bind_text(stmt, 3, from_address->user->device_id, -1, SQLITE_TRANSIENT);
    sqlite3_bind_int(stmt, 4, true);

    // step
    sqlite_step(stmt, SQLITE_ROW);

    // load
    int n_resending_plaintext = (int)sqlite3_column_int(stmt, 0);

    // release
    sqlite_finalize(stmt);

    return n_resending_plaintext;
}
...
size_t load_resending_plaintext(
    ...
    // allocate memory
    size_t n_resending_plaintext = load_n_resending_plaintext(from_address);
    if (n_resending_plaintext == 0){
        *e2ee_plaintext_data_list = NULL;
        *e2ee_plaintext_data_len_list = NULL;
        return 0;
    }
    *to_addresses = (Skissm__E2eeAddress **)malloc(sizeof(Skissm__E2eeAddress *) * n_resending_plaintext);

    *e2ee_plaintext_data_list = (uint8_t **)malloc(sizeof(uint8_t *) * n_resending_plaintext);

    *e2ee_plaintext_data_len_list = (size_t *)malloc(sizeof(size_t) * n_resending_plaintext);

    // prepare
    sqlite3_stmt *stmt;
    sqlite_prepare(RESENDING_PLAINTEXT_DATA_LOAD, &stmt);
    sqlite3_bind_text(stmt, 1, from_address->domain, -1, SQLITE_TRANSIENT);
    sqlite3_bind_text(stmt, 2, from_address->user->user_id, -1, SQLITE_TRANSIENT);
    sqlite3_bind_text(stmt, 3, from_address->user->device_id, -1, SQLITE_TRANSIENT);
    sqlite3_bind_int(stmt, 4, true);
    ...
    // step
    size_t i;
    for (i = 0; i < n_resending_plaintext; i++) {
        sqlite3_step(stmt);

        // load
        (*to_addresses)[i] = (Skissm__E2eeAddress *)malloc(sizeof(Skissm__E2eeAddress));
        skissm__e2ee_address__init((*to_addresses)[i]);
        (*to_addresses)[i]->user = (Skissm__PeerUser *)malloc(sizeof(Skissm__PeerUser));
        skissm__peer_user__init((*to_addresses)[i]->user);
        (*to_addresses)[i]->peer_case = SKISSM__E2EE_ADDRESS__PEER_USER;

        (*to_addresses)[i]->domain = strdup((char *)sqlite3_column_text(stmt, 0));
        (*to_addresses)[i]->user->user_id = strdup((char *)sqlite3_column_text(stmt, 1));
        (*to_addresses)[i]->user->device_id = strdup((char *)sqlite3_column_text(stmt, 2));

        size_t e2ee_plaintext_data_len = sqlite3_column_bytes(stmt, 3);
        uint8_t *e2ee_plaintext_data = (uint8_t *)sqlite3_column_blob(stmt, 3);
