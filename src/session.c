...
static void create_session_id(Org__E2eelab__Lib__Protobuf__E2eeSession *session) {
...
static bool check_message_fields(
    Org__E2eelab__Lib__Protobuf__E2eePreKeyPayload *pre_key_context,
...
void initialise_session(
    ...
    org__e2eelab__lib__protobuf__e2ee_session__init(session);
    copy_address_from_address(&(session->from), from);
    copy_address_from_address(&(session->to), to);
    initialise_ratchet(&(session->ratchet));
}

static void close_session(Org__E2eelab__Lib__Protobuf__E2eeSession *session){
    if (session != NULL){
        org__e2eelab__lib__protobuf__e2ee_session__free_unpacked(session, NULL);
    }
}
...
void pack_e2ee_plaintext(
    ...
    Org__E2eelab__Lib__Protobuf__E2eePlaintext *e2ee_plaintext = (Org__E2eelab__Lib__Protobuf__E2eePlaintext *) malloc(sizeof(Org__E2eelab__Lib__Protobuf__E2eePlaintext));
    org__e2eelab__lib__protobuf__e2ee_plaintext__init(e2ee_plaintext);
    // TODO: (*e2ee_plaintext)->version
    ...
    *context_len = org__e2eelab__lib__protobuf__e2ee_plaintext__get_packed_size(e2ee_plaintext);
    *context = (uint8_t *) malloc(sizeof(uint8_t) * (*context_len));
    org__e2eelab__lib__protobuf__e2ee_plaintext__pack(e2ee_plaintext, *context);
}
...
size_t new_outbound_session(
    ...
    Org__E2eelab__Lib__Protobuf__KeyPair my_ephemeral_key;
    CIPHER.suit1->gen_key_pair(&my_ephemeral_key);

    // Generate a new random ratchet key pair
    Org__E2eelab__Lib__Protobuf__KeyPair my_ratchet_key;
    CIPHER.suit1->gen_key_pair(&my_ratchet_key);

    const Org__E2eelab__Lib__Protobuf__KeyPair my_identity_key_pair =
    ...
    unset((void volatile *)&my_ephemeral_key, sizeof(Org__E2eelab__Lib__Protobuf__KeyPair));
    free_protobuf(&(my_ratchet_key.private_key));
    free_protobuf(&(my_ratchet_key.public_key));
    unset((void volatile *)&my_ratchet_key, sizeof(Org__E2eelab__Lib__Protobuf__KeyPair));
...
size_t new_inbound_session(
    ...
    Org__E2eelab__Lib__Protobuf__E2eeMessage *inbound_message
) {
    session->version = inbound_message->version;

    Org__E2eelab__Lib__Protobuf__E2eePreKeyPayload *pre_key_context = org__e2eelab__lib__protobuf__e2ee_pre_key_payload__unpack(NULL, inbound_message->payload.len, inbound_message->payload.data);
    ...
    const Org__E2eelab__Lib__Protobuf__OneTimePreKeyPair *our_one_time_pre_key;
    if (x3dh_epoch == 4){
        our_one_time_pre_key = lookup_one_time_pre_key(local_account, session->bob_one_time_pre_key);

        if (!our_one_time_pre_key){
            ssm_notify_error(BAD_MESSAGE_KEY_ID, "new_inbound_session()");
            return (size_t)(-1);
        } else{
    ...
    const Org__E2eelab__Lib__Protobuf__KeyPair *bob_identity_key = local_account->identity_key_pair;
    const Org__E2eelab__Lib__Protobuf__KeyPair *bob_signed_pre_key = local_account->signed_pre_key_pair->key_pair;
    const Org__E2eelab__Lib__Protobuf__KeyPair *bob_one_time_pre_key;
    if (x3dh_epoch == 4){
...
static size_t perform_encrypt_session(
    ...
    Org__E2eelab__Lib__Protobuf__E2eeMessage *outbound_message;
    outbound_message = (Org__E2eelab__Lib__Protobuf__E2eeMessage *) malloc(sizeof(Org__E2eelab__Lib__Protobuf__E2eeMessage));
    org__e2eelab__lib__protobuf__e2ee_message__init(outbound_message);
    ...
    Org__E2eelab__Lib__Protobuf__E2eeMsgPayload *msg_context = NULL;

    if (session->responded){
        outbound_message->msg_type = ORG__E2EELAB__LIB__PROTOBUF__E2EE_MESSAGE_TYPE__MESSAGE;
        encrypt_ratchet(
            session->ratchet,
        ...
        size_t msg_len = org__e2eelab__lib__protobuf__e2ee_msg_payload__get_packed_size(msg_context);
        outbound_message->payload.len = msg_len;
        outbound_message->payload.data = (uint8_t *) malloc(sizeof(uint8_t) * msg_len);
        org__e2eelab__lib__protobuf__e2ee_msg_payload__pack(msg_context, outbound_message->payload.data);
    } else {
        outbound_message->msg_type = ORG__E2EELAB__LIB__PROTOBUF__E2EE_MESSAGE_TYPE__PRE_KEY;
        Org__E2eelab__Lib__Protobuf__E2eePreKeyPayload *pre_key_context = (Org__E2eelab__Lib__Protobuf__E2eePreKeyPayload *) malloc(sizeof(Org__E2eelab__Lib__Protobuf__E2eePreKeyPayload));
        org__e2eelab__lib__protobuf__e2ee_pre_key_payload__init(pre_key_context);
        ...
        size_t pre_key_len = org__e2eelab__lib__protobuf__e2ee_pre_key_payload__get_packed_size(pre_key_context);
        outbound_message->payload.len = pre_key_len;
        outbound_message->payload.data = (uint8_t *) malloc(sizeof(uint8_t) * pre_key_len);
        org__e2eelab__lib__protobuf__e2ee_pre_key_payload__pack(pre_key_context, outbound_message->payload.data);
    }
    ...
    protocol_msg->cmd = ORG__E2EELAB__LIB__PROTOBUF__E2EE_COMMANDS__e2ee_msg;

    protocol_msg->payload.len = org__e2eelab__lib__protobuf__e2ee_message__get_packed_size(outbound_message);
    protocol_msg->payload.data = (uint8_t *) malloc(protocol_msg->payload.len);
    org__e2eelab__lib__protobuf__e2ee_message__pack(outbound_message, protocol_msg->payload.data);
    ...
    org__e2eelab__lib__protobuf__e2ee_message__free_unpacked(outbound_message, NULL);

    if (message_len == (size_t)(-1)) {
        ssm_notify_error(BAD_MESSAGE_ENCRYPTION, "encrypt_session()");
        return message_len;
    }
...
static void handle_pre_key_bundle_response(
    ...
    Org__E2eelab__Lib__Protobuf__E2eeSession *session = (Org__E2eelab__Lib__Protobuf__E2eeSession *) malloc(sizeof(Org__E2eelab__Lib__Protobuf__E2eeSession));
    initialise_session(session, from, to);
    copy_address_from_address(&(session->session_owner), from);
    Org__E2eelab__Lib__Protobuf__E2eeAccount *local_account = get_local_account(from);
...
size_t encrypt_session(
    ...
    Org__E2eelab__Lib__Protobuf__E2eeSession *session = NULL;
    ssm_handler.load_outbound_session(from, to, &session);
    if (session == NULL){
        pre_key_bundle_handler.from = from;
        pre_key_bundle_handler.to = to;
        pre_key_bundle_handler.plaintext = (uint8_t *)context;
        pre_key_bundle_handler.plaintext_len = context_len;
...
size_t decrypt_session(
    ...
    Org__E2eelab__Lib__Protobuf__E2eePreKeyPayload *pre_key_context = NULL;
    Org__E2eelab__Lib__Protobuf__E2eeMsgPayload *msg_payload = NULL;

    if (inbound_message->msg_type != ORG__E2EELAB__LIB__PROTOBUF__E2EE_MESSAGE_TYPE__PRE_KEY
        && inbound_message->msg_type != ORG__E2EELAB__LIB__PROTOBUF__E2EE_MESSAGE_TYPE__MESSAGE
    ) {
    ...
    if (session == NULL){
        ...
        session = (Org__E2eelab__Lib__Protobuf__E2eeSession *) malloc(sizeof(Org__E2eelab__Lib__Protobuf__E2eeSession));
        ...
        Org__E2eelab__Lib__Protobuf__E2eeAccount *local_account = get_local_account(inbound_message->to);
        new_inbound_session(session, local_account, inbound_message);

        if (compare_protobuf(&(session->session_id), &(inbound_message->session_id)) == false){
            close_session(session);
            ssm_notify_error(BAD_MESSAGE_FORMAT, "decrypt_session()");
            return (size_t)(-1);
        }
    ...
    if (inbound_message->msg_type == ORG__E2EELAB__LIB__PROTOBUF__E2EE_MESSAGE_TYPE__MESSAGE){
        msg_payload = org__e2eelab__lib__protobuf__e2ee_msg_payload__unpack(NULL, inbound_message->payload.len, inbound_message->payload.data);
    } else if (inbound_message->msg_type == ORG__E2EELAB__LIB__PROTOBUF__E2EE_MESSAGE_TYPE__PRE_KEY){
        pre_key_context = org__e2eelab__lib__protobuf__e2ee_pre_key_payload__unpack(NULL, inbound_message->payload.len, inbound_message->payload.data);
    ...
    if (msg_payload != NULL) {
        ...
        if (context_len == (size_t)(-1)){
            ...
            Org__E2eelab__Lib__Protobuf__E2eePlaintext *e2ee_plaintext = org__e2eelab__lib__protobuf__e2ee_plaintext__unpack(NULL, context_len, context);
            if (e2ee_plaintext->plaintext_type == ORG__E2EELAB__LIB__PROTOBUF__E2EE_PLAINTEXT_TYPE__COMMON_MSG){
                ssm_notify_one2one_msg(inbound_message->from, inbound_message->to, e2ee_plaintext->payload.data, e2ee_plaintext->payload.len);
            } else if (e2ee_plaintext->plaintext_type == ORG__E2EELAB__LIB__PROTOBUF__E2EE_PLAINTEXT_TYPE__GROUP_PRE_KEY){
                Org__E2eelab__Lib__Protobuf__E2eeGroupPreKeyPayload *group_pre_key_payload = org__e2eelab__lib__protobuf__e2ee_group_pre_key_payload__unpack(NULL, e2ee_plaintext->payload.len, e2ee_plaintext->payload.data);
        ...
        Org__E2eelab__Lib__Protobuf__E2eeSession *outbound_session = NULL;
...
complete:
    // release
    if (inbound_message->msg_type == ORG__E2EELAB__LIB__PROTOBUF__E2EE_MESSAGE_TYPE__MESSAGE){
        org__e2eelab__lib__protobuf__e2ee_msg_payload__free_unpacked(msg_payload, NULL);
    } else if (inbound_message->msg_type == ORG__E2EELAB__LIB__PROTOBUF__E2EE_MESSAGE_TYPE__PRE_KEY){
        org__e2eelab__lib__protobuf__e2ee_pre_key_payload__free_unpacked(pre_key_context, NULL);
    }
    close_session(session);
