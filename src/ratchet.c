...
void initialise_as_bob(
    ...
    if (cipher_suite->get_crypto_param().pqc_param == false) {
        ...
        uint8_t temp_secret[shared_key_len];
        sender_chain->our_ratchet_public_key.data = cipher_suite->ss_key_gen(NULL, their_ratchet_key, temp_secret);
...
size_t decrypt_ratchet(
    ...
    if (receiver_chain == NULL && skipped_message == false) {
        ...
        if (cipher_suite->get_crypto_param().pqc_param == false) {
            ...
            cipher_suite->asym_key_gen(&(new_ratchet_key_pair->public_key), &(new_ratchet_key_pair->private_key));
