...
size_t consume_get_pre_key_bundle_response_payload(
    ...
    Skissm__PreKeyBundle *their_pre_key_bundle = get_pre_key_bundle_response_payload->pre_key_bundle;
...
Skissm__E2eeMsg *produce_e2ee_message_payload(Skissm__Session *outbound_session, const uint8_t *e2ee_plaintext_data, size_t e2ee_plaintext_len) {
    ...
    outbound_e2ee_message_payload->msg_type = SKISSM__E2EE_MSG_TYPE__MESSAGE;
...
size_t consume_e2ee_message_payload(Skissm__E2eeMsg *inbound_e2ee_message_payload) {
    ...
    if (inbound_e2ee_message_payload->msg_type != SKISSM__E2EE_MSG_TYPE__MESSAGE) {
    ...
    if (msg_payload != NULL) {
        ...
        if (context_len == (size_t)(-1)) {
            ...
            Skissm__Plaintext *e2ee_plaintext = skissm__plaintext__unpack(NULL, context_len, context);
            if (e2ee_plaintext != NULL) {
                if (e2ee_plaintext->type == SKISSM__PLAINTEXT_TYPE__COMMON_MSG) {
                ...
                } else if (e2ee_plaintext->type == SKISSM__PLAINTEXT_TYPE__GROUP_PRE_KEY) {
                    Skissm__GroupPreKeyPayload *group_pre_key_payload = skissm__group_pre_key_payload__unpack(NULL, e2ee_plaintext->payload.len, e2ee_plaintext->payload.data);
                ...
                skissm__plaintext__free_unpacked(e2ee_plaintext, NULL);
            } else {
...
Skissm__InvitePayload *produce_e2ee_invite_payload(
    ...
    Skissm__InvitePayload *invite_payload = (Skissm__InvitePayload *) malloc(sizeof(Skissm__InvitePayload));
    skissm__invite_payload__init(invite_payload);

    invite_payload->e2ee_pack_id = outbound_session->e2ee_pack_id;

    copy_protobuf_from_protobuf(&(invite_payload->alice_identity_key), &(outbound_session->alice_identity_key));
    if (pre_shared_key_2 == NULL){
        // In the ECC version, the ephemeral public key will be sent
        invite_payload->n_pre_shared_key = 1;
        invite_payload->pre_shared_key = (ProtobufCBinaryData *) malloc(sizeof(ProtobufCBinaryData));
        copy_protobuf_from_protobuf(invite_payload->pre_shared_key, pre_shared_key_1);
    } else{
        // In the PQC version, three "ciphertexts" will be sent
        size_t pre_shared_key_len = pre_shared_key_1->len;
        invite_payload->n_pre_shared_key = 3;
        invite_payload->pre_shared_key = (ProtobufCBinaryData *) malloc(sizeof(ProtobufCBinaryData) * 3);
        copy_protobuf_from_protobuf(invite_payload->pre_shared_key, pre_shared_key_1);
        copy_protobuf_from_protobuf(invite_payload->pre_shared_key + pre_shared_key_len, pre_shared_key_2);
        copy_protobuf_from_protobuf(invite_payload->pre_shared_key + pre_shared_key_len + pre_shared_key_len, pre_shared_key_3);
    }
    invite_payload->bob_signed_pre_key_id = outbound_session->bob_signed_pre_key_id;
    invite_payload->bob_one_time_pre_key_id = outbound_session->bob_one_time_pre_key_id;

    return invite_payload;
}

size_t consume_e2ee_invite_payload(Skissm__E2eeMsg *invite_msg_payload){
    Skissm__InvitePayload *invite_payload = skissm__invite_payload__unpack(NULL, invite_msg_payload->payload.len, invite_msg_payload->payload.data);
    uint32_t e2ee_pack_id = invite_payload->e2ee_pack_id;
    ...
    Skissm__Account *local_account = get_local_account(invite_msg_payload->to);
    const session_suite_t *session_suite = get_e2ee_pack(e2ee_pack_id)->session_suite;
    ...
    // create a new inbound session
    size_t result = session_suite->new_inbound_session(inbound_session, local_account, invite_payload);
...
Skissm__AcceptPayload *produce_e2ee_accept_payload(uint32_t e2ee_pack_id, ProtobufCBinaryData *ciphertext_1) {
    Skissm__AcceptPayload *accept_payload = (Skissm__AcceptPayload *) malloc(sizeof(Skissm__AcceptPayload));
    skissm__accept_payload__init(accept_payload);

    accept_payload->e2ee_pack_id = e2ee_pack_id;

    if (ciphertext_1 == NULL){
        accept_payload->n_pre_shared_key = 0;
        accept_payload->pre_shared_key = NULL;
    } else{
        accept_payload->n_pre_shared_key = 1;
        accept_payload->pre_shared_key = (ProtobufCBinaryData *) malloc(sizeof(ProtobufCBinaryData) * 1);
        copy_protobuf_from_protobuf(accept_payload->pre_shared_key, ciphertext_1);
    }
...
size_t consume_e2ee_accept_payload(Skissm__E2eeMsg *accept_msg_payload){
    ...
    Skissm__AcceptPayload *accept_payload = skissm__accept_payload__unpack(NULL, accept_msg_payload->payload.len, accept_msg_payload->payload.data);
    const session_suite_t *session_suite = get_e2ee_pack(accept_payload->e2ee_pack_id)->session_suite;
    size_t result = session_suite->complete_outbound_session(outbound_session, accept_payload);
...
Skissm__E2eeMsg *produce_invite_message_payload(Skissm__Session *outbound_session, Skissm__InvitePayload *invite_payload){
    ...
    invite_message_payload->msg_type = SKISSM__E2EE_MSG_TYPE__INVITE;
    ...
    invite_message_payload->payload.len = skissm__invite_payload__get_packed_size(invite_payload);
    invite_message_payload->payload.data = (uint8_t *)malloc(sizeof(uint8_t) * invite_message_payload->payload.len);
    skissm__invite_payload__pack(invite_payload, invite_message_payload->payload.data);
...
Skissm__E2eeMsg *produce_accept_message_payload(Skissm__E2eeAddress *from, Skissm__E2eeAddress *to, Skissm__AcceptPayload *accept_payload){
    ...
    accept_message_payload->msg_type = SKISSM__E2EE_MSG_TYPE__ACCEPT;

    copy_address_from_address(&(accept_message_payload->from), from);
    copy_address_from_address(&(accept_message_payload->to), to);

    accept_message_payload->payload.len = skissm__accept_payload__get_packed_size(accept_payload);
    accept_message_payload->payload.data = (uint8_t *)malloc(sizeof(uint8_t) * accept_message_payload->payload.len);
    skissm__accept_payload__pack(accept_payload, accept_message_payload->payload.data);
