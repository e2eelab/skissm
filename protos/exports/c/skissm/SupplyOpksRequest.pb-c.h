/* Generated by the protocol buffer compiler.  DO NOT EDIT! */
/* Generated from: SupplyOpksRequest.proto */

#ifndef PROTOBUF_C_SupplyOpksRequest_2eproto__INCLUDED
#define PROTOBUF_C_SupplyOpksRequest_2eproto__INCLUDED

#include <protobuf-c/protobuf-c.h>

PROTOBUF_C__BEGIN_DECLS

#if PROTOBUF_C_VERSION_NUMBER < 1003000
# error This file was generated by a newer version of protoc-c which is incompatible with your libprotobuf-c headers. Please update your headers.
#elif 1004000 < PROTOBUF_C_MIN_COMPILER_VERSION
# error This file was generated by an older version of protoc-c which is incompatible with your libprotobuf-c headers. Please regenerate this file with a newer version of protoc-c.
#endif

#include "skissm/E2eeAddress.pb-c.h"
#include "skissm/OneTimePreKeyPublic.pb-c.h"

typedef struct Skissm__SupplyOpksRequest Skissm__SupplyOpksRequest;


/* --- enums --- */


/* --- messages --- */

struct  Skissm__SupplyOpksRequest
{
  ProtobufCMessage base;
  char *e2ee_pack_id;
  Skissm__E2eeAddress *user_address;
  size_t n_one_time_pre_key_public;
  Skissm__OneTimePreKeyPublic **one_time_pre_key_public;
};
#define SKISSM__SUPPLY_OPKS_REQUEST__INIT \
 { PROTOBUF_C_MESSAGE_INIT (&skissm__supply_opks_request__descriptor) \
    , (char *)protobuf_c_empty_string, NULL, 0,NULL }


/* Skissm__SupplyOpksRequest methods */
void   skissm__supply_opks_request__init
                     (Skissm__SupplyOpksRequest         *message);
size_t skissm__supply_opks_request__get_packed_size
                     (const Skissm__SupplyOpksRequest   *message);
size_t skissm__supply_opks_request__pack
                     (const Skissm__SupplyOpksRequest   *message,
                      uint8_t             *out);
size_t skissm__supply_opks_request__pack_to_buffer
                     (const Skissm__SupplyOpksRequest   *message,
                      ProtobufCBuffer     *buffer);
Skissm__SupplyOpksRequest *
       skissm__supply_opks_request__unpack
                     (ProtobufCAllocator  *allocator,
                      size_t               len,
                      const uint8_t       *data);
void   skissm__supply_opks_request__free_unpacked
                     (Skissm__SupplyOpksRequest *message,
                      ProtobufCAllocator *allocator);
/* --- per-message closures --- */

typedef void (*Skissm__SupplyOpksRequest_Closure)
                 (const Skissm__SupplyOpksRequest *message,
                  void *closure_data);

/* --- services --- */


/* --- descriptors --- */

extern const ProtobufCMessageDescriptor skissm__supply_opks_request__descriptor;

PROTOBUF_C__END_DECLS


#endif  /* PROTOBUF_C_SupplyOpksRequest_2eproto__INCLUDED */
